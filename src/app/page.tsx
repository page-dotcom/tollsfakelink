...
// PIN Kamu (Tetap pakai ini untuk keamanan API)
const MY_PIN = "Yasue1998"; 

export default function Home() {
  // --- STATE (Penyimpanan Data Sementara) ---
  const [settings, setSettings] = useState({
    site_name: 'ShortCuts - Personal Tool',
    offer_url: '',
    histats_id: ''
  });
  ...
  useEffect(() => {
    ...
    // Cek LocalStorage (Fitur JS lama kamu)
  ...
  async function fetchSettings() {
    ...
    if (data) {
      setSettings(data);
      document.title = data.site_name; // Ubah Judul Tab
    }
  }
  ...
  // --- 2. LOGIKA MEMBUAT LINK (Shorten) ---
  const handleShorten = async (e: FormEvent) => {
    e.preventDefault();
    if (!longUrl) return;

    // Efek Transisi jQuery .hide() -> .fadeIn() disimulasikan dengan state
    ...
    try {
      ...
      setTimeout(() => {
        if (data.success) {
          const result = `https://tollsfakelink.vercel.app/${data.shortId}`; // Ganti domain jika perlu
          setShortUrl(result);
          localStorage.setItem('lastLink', result);
          setViewState('result'); // Tampilkan hasil
          setLongUrl("");
          fetchLinks(); // Refresh tabel
        } else {
          setViewState('form');
          // Error handling simpel (bisa ganti text di bawah input kalau mau)
  ...
  const handleCopy = (text: string, isListBtn = false, id = "") => {
    ...
    if (!isListBtn) {
      ...
      // Tombol di List (Manipulasi DOM manual untuk performa list)
      const btnIcon = document.getElementById(`icon-copy-${id}`);
      if(btnIcon) {
        btnIcon.className = "glyphicon glyphicon-ok"; // Ubah jadi centang
        setTimeout(() => {
          btnIcon.className = "glyphicon glyphicon-copy"; // Balikin
        }, 1500);
      }
    }
  };

  // --- 4. LOGIKA EDIT & DELETE ---
  const openEdit = (id: string, currentUrl: string) => {
    setEditingId(id);
    setEditUrlVal(currentUrl);
    // Scroll ke atas sedikit biar kelihatan
  ...
  const handleDelete = async (id: string) => {
    // Kita pakai confirm bawaan (tapi bisa diganti custom modal kalau mau strict no-js alert)
    // Untuk saat ini biar cepat pakai confirm dulu, atau hapus baris UI kayak jQuery tadi
  ...
  // --- RENDER HTML (PERSIS STRUKTUR KAMU) ---
  return (
    <>
      {/* NAVBAR */}
          ...
          <div className="col-md-8 col-md-offset-2 col-sm-10 col-sm-offset-1 col-xs-12">
            ...
            <div className="tool-box">
              ...
              <div className="tool-body">
                ...
                {viewState === 'result' && (
                  <div id="result-view">
                    <div className="input-group input-group-lg">
                      ...
                      <span className="input-group-btn">
                        <button 
                          className={`btn btn-lg-custom ${copyBtnText === 'COPIED!' ? 'btn-copy-success' : 'btn-copy-default'}`} 
                          onClick={() => handleCopy(shortUrl)} 
                          type="button"
                          style={{ background: copyBtnText === 'COPIED!' ? '#27ae60' : '' }}
                        >
                          {copyBtnText}
                        </button>
                      </span>
                ...
                {/* MODAL EDIT (Menyatu dengan desain) */}
                {editingId && (
                  <div className="edit-view" style={{textAlign:'left', padding:'10px'}}>
                    <label style={{fontSize:'12px', color:'#999'}}>EDIT TUJUAN URL:</label>
                    <div className="input-group">
                      ...
                      />
                      <span className="input-group-btn">
                        <button className="btn btn-success" type="button" onClick={saveEdit}>SAVE</button>
                      </span>
                    </div>
            ...
            <div className="text-center">
              <button 
                ...
                id="btnShowList"
              >
                <span className="glyphicon glyphicon-list-alt"></span> MY URL LIST
              </button>
            </div>

            {/* URL LIST AREA (TABEL) */}
            {showList && (
              <div className="list-box" id="urlListArea" style={{display:'block', marginTop:'20px'}}>
                    ...
                    <tbody id="listBody">
                      {links.length === 0 ? (
                        ...
                        links.map(link => (
                          <tr key={link.id}>
                            <td>
                              {/* GAMBAR OTOMATIS DARI GOOGLE FAVICON */}
                              <img 
                                src={getFavicon(link.url)} 
                                alt="icon" 
                                className="img-thumb" 
                                style={{width:'35px', height:'35px', borderRadius:'8px', objectFit:'cover'}}
                                onError={(e) => { e.currentTarget.style.display='none' }} // Sembunyi kalau error
                              />
                            </td>
                            <td>
                            ...
                            <td className="text-right">
                              {/* TOMBOL COPY LIST */}
                              <button 
                                className="btn-action-icon btn-copy-list" 
                                title="Copy" 
                                onClick={() => handleCopy(`https://tollsfakelink.vercel.app/${link.id}`, true, link.id)}
                              >
                              ...
                              <button 
                                ...
                                title="Edit" 
                                onClick={() => openEdit(link.id, link.url)}
                              >
                              ...
                              {/* TOMBOL HAPUS */}
                              <button 
                                className="btn-action-icon btn-del" 
                                title="Delete" 
                ...
                {/* PAGINATION (Statis dulu sesuai HTML kamu) */}
                <div className="pagination-area" style={{textAlign:'center', marginTop:'15px'}}>
                   <button type="button" className="btn btn-default btn-sm btn-nav" disabled>
                       <span className="glyphicon glyphicon-chevron-left"></span> Prev
                   </button>
                   <span className="page-count" style={{margin:'0 10px'}}>Page 1</span>
                   <button type="button" className="btn btn-default btn-sm btn-nav">
            ...
            <div className="text-center" style={{marginTop: '30px', marginBottom: '20px'}}>
              <button 
                type="button" 
                className={`btn-gear-toggle ${showSettings ? 'active' : ''}`} 
            ...
            {/* SETTINGS PANEL */}
            {showSettings && (
              <div className="settings-panel" id="settingsArea" style={{display:'block'}}>
                ...
                <div className="settings-body">
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-group">
                      ...
                      <textarea 
                        className="form-control" 
                        rows={2} 
                        value={settings.offer_url} 
                        onChange={(e) => setSettings({...settings, offer_url: e.target.value})} 
                        placeholder="https://..."
                      ></textarea>
                    </div>
                    <p className="help-block" style={{fontSize:'12px', color:'#ccc', marginTop:'5px'}}>Link tujuan jika visitor membuka root domain.</p>
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-group">
                      ...
                      <input 
                        type="number" 
                        className="form-control" 
                        value={settings.histats_id} 
                        onChange={(e) => setSettings({...settings, histats_id: e.target.value})} 
                  ...
                  <div style={{marginTop: '25px'}}>
                    <button 
                      ...
                      style={{ background: saveBtnColor, color: '#fff' }}
                    >
                      {saveBtnText}
                    </button>
                  </div>
