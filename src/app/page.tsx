...
export default function Home() {
  ...
  const [saveBtnColor, setSaveBtnColor] = useState(""); // Default kosong
  ...
  const handleDelete = async (id: string) => {
    // Hapus langsung dari UI biar cepat
        ...
        <div className="container-fluid">
          <div className="navbar-header">
            <a className="navbar-brand" href="#">
              <svg className="brand-icon-svg" viewBox="0 0 24 24" style={{width:32, height:32, fill:'#3498db', marginRight:10}}>
          ...
          <div className="col-md-8 col-md-offset-2 col-sm-10 col-sm-offset-1 col-xs-12">
            ...
            <div className="tool-box">
              ...
              <div className="tool-body">
                ...
                {!editingId && viewState === 'result' && (
                  <div id="result-view" style={{display:'block'}}>
                    <div className="input-group input-group-lg">
                      ...
                      <span className="input-group-btn">
                        <button 
                          className={`btn btn-lg-custom ${copyBtnText === 'COPIED!' ? 'btn-success' : ''}`}
                          id="btnCopy" 
                          type="button" 
                ...
                {/* SHARE AREA (DI DALAM TOOL-BODY SESUAI HTML ASLI) */}
                {/* Kita paksa muncul jika result view aktif */}
                {!editingId && viewState === 'result' && (
                  <div id="share-area" style={{display:'block', marginTop:25}}>
                    <div className="social-icons" style={{display:'flex', justifyContent:'center', gap:10}}>
                      {/* WA */}
                      <a href={`https://api.whatsapp.com/send?text=${shortUrl}`} target="_blank" className="bg-wa" style={{width:40, height:40, borderRadius:'50%', background:'#25D366', display:'flex', alignItems:'center', justifyContent:'center'}}>
                      ...
                      {/* FB */}
                      <a href={`https://www.facebook.com/sharer/sharer.php?u=${shortUrl}`} target="_blank" className="bg-fb" style={{width:40, height:40, borderRadius:'50%', background:'#1877F2', display:'flex', alignItems:'center', justifyContent:'center'}}>
                         <svg viewBox="0 0 24 24" style={{width:20, height:20, fill:'#fff'}}><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                      </a>
                      {/* TW */}
                      <a href={`https://twitter.com/intent/tweet?url=${shortUrl}`} target="_blank" className="bg-tw" style={{width:40, height:40, borderRadius:'50%', background:'#1DA1F2', display:'flex', alignItems:'center', justifyContent:'center'}}>
                         <svg viewBox="0 0 24 24" style={{width:20, height:20, fill:'#fff'}}><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>
                      </a>
                      {/* TG */}
                      <a href={`https://t.me/share/url?url=${shortUrl}`} target="_blank" className="bg-tg" style={{width:40, height:40, borderRadius:'50%', background:'#0088cc', display:'flex', alignItems:'center', justifyContent:'center'}}>
                         <svg viewBox="0 0 24 24" style={{width:20, height:20, fill:'#fff'}}><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
                      </a>
                    </div>
                  </div>
                )}
                
                {/* EDIT FORM (Diselipkan disini agar layout sama) */}
                {editingId && (
                   <div className="edit-view" style={{marginTop:20}}>
                     <div className="input-group">
                       ...
                       <span className="input-group-btn">
                         <button className="btn btn-success" type="button" onClick={saveEdit} style={{height:55}}>SAVE</button>
                       </span>
                     </div>
              ...
              <div className="list-box" id="urlListArea" style={{display:'block', marginTop:20, background:'#fff', borderRadius:12, padding:20, boxShadow:'0 5px 20px rgba(0,0,0,0.05)'}}>
                <div className="table-responsive">
                  <table className="table table-hover">
                    ...
                    <tbody id="listBody">
                       {links.length === 0 ? (
                         <tr><td colSpan={6} className="text-center" style={{padding:20}}>Belum ada link.</td></tr>
                       ) : (
                         links.map(link => (
                           <tr key={link.id}>
                             <td>
                ...
                <div className="pagination-area" style={{textAlign:'center', marginTop:15}}>
                  <button className="btn btn-default btn-sm btn-nav" disabled><span className="glyphicon glyphicon-chevron-left"></span> Prev</button>
                  <span className="page-count" style={{margin:'0 10px'}}>Page 1</span>
                  <button className="btn btn-default btn-sm btn-nav">Next <span className="glyphicon glyphicon-chevron-right"></span></button>
                </div>
              </div>
            )}
