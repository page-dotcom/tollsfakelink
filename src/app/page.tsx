...
export default function Home() {
  ...
  const [showList, setShowList] = useState(false); // Default false (tersembunyi)
  const [showSettings, setShowSettings] = useState(false);

  // Feedback Text Tombol
  ...
  async function fetchLinks() {
    try {
      ...
      if (json.success) {
        setLinks(json.data); // Data masuk ke state links
      }
    ...
    try {
      ...
      setTimeout(() => {
        if (data.success) {
          ...
          setViewState('result');
          setLongUrl("");
          fetchLinks(); // Update tabel otomatis
        } else {
          setViewState('form');
  ...
  // Helper: Ambil Favicon
  const getFavicon = (url: string) => {
    try {
      const domain = new URL(url).hostname;
      return `https://www.google.com/s2/favicons?domain=${domain}&sz=64`;
    } catch { return "https://www.google.com/s2/favicons?domain=google.com"; }
  };

  // --- RENDER HTML SESUAI PERMINTAAN ---
  return (
    <>
      ...
      <div className="container">
        <div className="row">
          <div className="col-md-8 col-md-offset-2 col-sm-10 col-sm-offset-1 col-xs-12">
              ...
              <div className="tool-body">
                ...
                {!editingId && viewState === 'result' && (
                  <div id="result-view">
                    ...
                    {/* SHARE AREA (Sesuai SVG HTML kamu) */}
                    <div id="share-area">
                ...
                {/* EDIT FORM (Muncul jika ada ID yang diedit) */}
                {editingId && (
            ...
            {/* URL LIST AREA (INI YANG GAK MUNCUL KEMARIN) */}
            {/* Kita pakai Conditional Rendering yang aman */}
            {showList && (
              <div className="list-box" id="urlListArea" style={{display:'block'}}>
                          ...
                          <tr key={link.id}>
                            <td>
                              <div className="img-thumb" style={{
                                backgroundImage: `url(${getFavicon(link.url)})`, 
                            ...
                            <td className="text-right">
                              <button className="btn-action-icon btn-copy-list" title="Copy" onClick={() => handleCopy(`https://tollsfakelink.vercel.app/${link.id}`, true, link.id)}>
                                <span id={`icon-copy-${link.id}`} className="glyphicon glyphicon-copy"></span>
                              </button>
                              <button className="btn-action-icon" title="Edit" onClick={() => openEdit(link.id, link.url)}>
                                <span className="glyphicon glyphicon-pencil"></span>
                              </button>
                              <button className="btn-action-icon btn-del" title="Delete" onClick={() => handleDelete(link.id)}>
                ...
                <div className="settings-body">
                  <div id="settingsForm">
                    ...
                    <div className="form-group">
                      ...
                      <p className="help-block">Link tujuan jika visitor membuka root domain.</p>
                    </div>
