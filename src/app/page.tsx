...
export default function Home() {
  ...
  return (
    <>
      {toast && (
        <div style={{position:'fixed', top:20, left:'50%', transform:'translateX(-50%)', background:'#333', color:'#fff', padding:'10px 20px', borderRadius:50, zIndex:99999}}>
          {toast}
        </div>
      )}
      ...
      {!session && (
        <div className="login-overlay">
          <div className="login-box">
            <h3 style={{marginTop:0, marginBottom:20}}>LOGIN DASHBOARD</h3>
            <form onSubmit={handleLogin}>
              <input className="form-control" type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} style={{marginBottom:15}} required />
              <input className="form-control" type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} style={{marginBottom:20}} required />
        ...
        <>
          <nav className="navbar-custom">
            <div className="container" style={{display:'flex', justifyContent:'space-between'}}>
              <a href="#" className="navbar-brand">
                <span className="material-icons-round">link</span> {settings.site_name}
              </a>
          ...
          <div className="container">
            <div style={{maxWidth:700, margin:'0 auto'}}>
              ...
              {/* PANEL UTAMA */}
              <div className="panel">
                <div className="panel-header">{editingId ? 'EDIT LINK' : 'BUAT SHORT LINK'}</div>
                  ...
                  {editingId ? (
                    <div>
                      ...
                      <div style={{display:'flex', gap:10}}>
                        ...
                        <button className="btn btn-outline btn-block" onClick={()=>setEditingId(null)}>BATAL</button>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      {/* LOADING */}
                      {viewState === 'loading' && (
                        <div style={{textAlign:'center', padding:20, color:'#777'}}>Sedang memproses...</div>
                      )}

                      {/* RESULT */}
                      {viewState === 'result' && (
                        <div className="result-container">
                          <div className="link-display">{shortUrl}</div>
                          <button className="btn btn-success btn-sm" onClick={()=>copyText(shortUrl)}>SALIN LINK</button>
                          
                          <div className="social-area">
                            <small style={{color:'#999'}}>Bagikan:</small>
                            <div className="social-icons">
                              ...
                              <a href={`https://www.facebook.com/sharer/sharer.php?u=${shortUrl}`} target="_blank" className="social-btn bg-fb"><svg viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
                              <a href={`https://twitter.com/intent/tweet?url=${shortUrl}`} target="_blank" className="social-btn bg-tw"><svg viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg></a>
                              <a href={`https://t.me/share/url?url=${shortUrl}`} target="_blank" className="social-btn bg-tg"><svg viewBox="0 0 24 24"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg></a>
                            </div>
                            <div style={{marginTop:15}}>
                              <a style={{cursor:'pointer', textDecoration:'underline', color:'#3498db', fontSize:14}} onClick={()=>{setViewState('input'); setLongUrl('')}}>Buat Lagi</a>
                            </div>
                          </div>
                        </div>
                      )}
              ...
              {/* TOGGLES */}
              <div style={{display:'flex', gap:10, justifyContent:'center', marginBottom:20}} className="toggle-btn-group">
                <button className="btn btn-outline" onClick={()=>setShowList(!showList)} style={{marginRight:10}}>
                ...
                <div className="panel">
                  ...
                  <div className="panel-body" style={{padding:0}}>
                        ...
                        <tbody>
                          {links.map(link => (
                            <tr key={link.id}>
                              ...
                              <td><div style={{maxWidth:150, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', color:'#777'}}>{link.url}</div></td>
                              <td><span style={{background:'#f1f1f1', padding:'2px 8px', borderRadius:3, fontSize:12}}>{link.clicks}</span></td>
                              <td style={{textAlign:'right', minWidth:120}}>
                                <button className="btn btn-success btn-xs" onClick={()=>copyText(`https://tollsfakelink.vercel.app/${link.id}`)}>C</button>
                                <button className="btn btn-primary btn-xs" onClick={()=>startEdit(link)}>E</button>
                                <button className="btn btn-xs btn-del" onClick={()=>handleDelete(link.id)}>X</button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    <div style={{padding:20, display:'flex', justifyContent:'center', gap:10}}>
                      <button className="btn btn-outline btn-xs" disabled={currentPage===1} onClick={()=>setCurrentPage(c=>c-1)}>Prev</button>
                      <span style={{fontSize:13, alignSelf:'center'}}>Halaman {currentPage}</span>
                      <button className="btn btn-outline btn-xs" disabled={currentPage>=totalPages} onClick={()=>setCurrentPage(c=>c+1)}>Next</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {showSettings && (
                <div className="panel">
                  ...
                  <div className="panel-body">
                    ...
                    <div style={{background:'#f8f9fa', padding:15, border:'1px solid #eee', borderRadius:4, marginBottom:15}}>
